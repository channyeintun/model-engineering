/**
 * Text preparation + chunking for pocket-tts generation.
 *
 * Ported verbatim from next-editor's `src/studio/tts/pocket/textPrep.ts`
 * (Apache-2.0, originally KevinAHM/pocket-tts-web@d0c0c79b7712256a32d691c67f20b8ae2e020d00).
 * The tokenizer is injected so this stays pure and testable.
 */

export function prepareTextPrompt(text, options) {
...
function splitTokenIdsIntoChunks(tokenizer, tokenIds, maxTokens) {
  const chunks = [];
...
export function splitIntoBestSentences(text, tokenizer, options) {
  ...
  for (const sentenceText of sentences) {
    ...
    if (sentenceTokenIds.length > options.maxTokenPerChunk) {
      ...
      chunks.push(...splitTokenIdsIntoChunks(tokenizer, sentenceTokenIds, options.maxTokenPerChunk));
      continue;
    }
